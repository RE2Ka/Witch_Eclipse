...
public sealed class DamageSquareSystem : EntitySystem
{
    ...
    private void OnMapInit(Entity<DamageSquareComponent> ent, ref ComponentStartup args)
    {
        ...
        _transform.SetCoordinates(ent, _map.GridTileToLocal(gridUid, grid, tile.GridIndices));
    }
    ...
    private void Damage(Entity<DamageSquareComponent> field)
    {
        ...
        var tile = _map.GetTileRef(grid, gridComp, xform.Coordinates);
        ...
        foreach (var target in lookup)
        {
            ...
            if (!IsTargetInsideDamageTile(target, grid, gridComp, tile.GridIndices, field.Comp.HitboxScale))
                continue;
    ...
    private bool IsTargetInsideDamageTile(
        EntityUid target,
        EntityUid grid,
        ...
        float hitboxScale)
    {
        if (!_transform.TryGetGridTilePosition(target, out var targetTile, gridComp)
            || targetTile != tileIndices)
            return false;

        var targetWorld = _transform.GetWorldPosition(target);
        var targetLocal = _map.WorldToLocal(grid, gridComp, targetWorld);
        var tileCenter = _map.GridTileToLocal(grid, gridComp, tileIndices).Position;

        var halfHitbox = gridComp.TileSize * Math.Clamp(hitboxScale, 0.05f, 1f) / 2f;
        var offset = Vector2.Abs(targetLocal - tileCenter);

        return offset.X <= halfHitbox
            && offset.Y <= halfHitbox;
    }
}
