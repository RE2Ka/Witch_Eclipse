...
public sealed class SupermatterSystem : SharedSupermatterSystem
{
        ...
        GasMixture mixture)
    {
        ...
        var radiationIntensity = sMcomponent.Power *
                                 Math.Max(0, 1f + powerTransmissionBonus / 10f)
                                 * sMcomponent.RadiationPowerModifier
                                 * _config.GetCVar(CCVars.SupermatterRadsModifier);
        _radiation.SetIntensity((uid, radcomponent), Math.Max(sMcomponent.MinimumRadiationIntensity, radiationIntensity));
