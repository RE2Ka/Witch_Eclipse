...
public sealed partial class DockingConsoleWindow : FancyWindow
{
    ...
    public event Action<int>? OnBeaconFTL;
    ...
    public DockingConsoleWindow(EntityUid owner)
    {
        ...
        DestinationTabs.SetTabTitle(0, Loc.GetString("docking-console-tab-destinations"));
        DestinationTabs.SetTabTitle(1, Loc.GetString("docking-console-tab-beacons"));
        DestinationTabs.OnTabChanged += _ => UpdateButton();
        ...
        Beacons.OnItemSelected += args =>
        {
            _selectedBeacon = args.ItemIndex;
            UpdateButton();
        };

        Beacons.OnItemDeselected += _ =>
        {
        ...
        {
            if (DestinationTabs.CurrentTab == 1 && _selectedBeacon is { } beaconIndex)
                OnBeaconFTL?.Invoke(beaconIndex);
            else if (_selected is {} index)
                OnFTL?.Invoke(index);
        };
    }
    ...
    private void UpdateBeacons(DockingConsoleState state)
    {
        if (Beacons.Count == state.Beacons.Count)
            return;

        Beacons.Clear();
        foreach (var beacon in state.Beacons)
            Beacons.AddItem(beacon.Name);
    }

    private void UpdateButton()
    {
        MiningFtlState state = MiningFtlState.Unknown;
        var hasSelection = DestinationTabs.CurrentTab == 1
            ? _selectedBeacon != null
            : _selected != null;
